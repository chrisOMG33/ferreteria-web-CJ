...
export class LoginService {
  //url= '/api/users';
  url = 'https://z3d43z3gtn.us-east-1.awsapprunner.com/users';
  ...
  authenticate(userNm: string, password: string) {
    ...
    return this.http.get<IAuth[]>(this.url).pipe(
      map((response) => {
        let user = response.filter(
          (item) => item.userName === userNm && item.password === password
        );
        if (user && user.length > 0) {
          if (user[0].isActive) {
            ...
            result.isError = true;
            result.errorMessage =
              'The user is not active. Please contact the system administrator';
          }
        } else {
          //let customError= {
          //error:{message_code:''}
